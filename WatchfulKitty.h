...
/* VMCS State */
typedef struct starting_GUEST_STATE {
	ULONG_PTR cs;
	ULONG_PTR ds;
	ULONG_PTR ss;
	ULONG_PTR es;
...
} MS_MTRR_RANGE, *Point_MS_MTRR_RANGE;

typedef struct _kobject_generi
{
	UINT16 Pad[3]; 
	UINT16 Limit;
	void* Base;
} kobject_generic, *point_kobject_generic; //8b total
...
typedef struct _hypervisor_state
{
	...
	UINT64 Cr3;   
	UINT64 Cr4;    
	UINT64 MsrGsBase; //4*8b = 32b
	UINT16 Tr; //2b
	UINT16 Ldtr; //2b
	UINT64 DebugControl;
	UINT64 KernelDr7;   //16b
	kobject_generic Idtr; //8b  
	kobject_generic Gdtr; //8b
} hypervisor_state, *point_hypervisor_state; //32 + 4 + 16 + 16 = 68 bytes
...
typedef struct _cpu_state
{
	union
	{
		__declspec(align(4096)) UINT8 OurMaxStackSize[KERNEL_STACK_SIZE]; //Suggested to be 24*1024 bytes
		//align for 4kb pages, use PAGE_SIZE or similar provided by compiler for dynamic paging cap
		//this is an msvc compiler intrinsic, on linux you must conform to gcc compiler specs
		//use macros for portable code
		//  #ifndef __linux__ (if not linux, you can also use if win) 
		//     #define DECLSPEC_ALIGN(x)   __declspec(align(x))
		//  #endif
		//unions in c++ are different then C, this says our data should match this array on the stack
		struct
		{
